...
import { makeBossBot, walkMap, leaveShop, shopStep, MAP_POLICIES, SANCTUARY_MAP_POLICIES } from "./bot-policy.mjs";
import { BALANCE } from "../src/game/balance.ts";

/**
 * `--sanctuary`: 성소 실험. 정예 유물을 떼고(정예=생선만) 유물을 성소 노드로 옮긴다
 * (`BALANCE.mapSanctuary`). 성소로 가는 "성소 노림"이 안 가는 정책들을 이기는지 본다.
 */
const ARGS = process.argv.slice(2);
const SANCTUARY = ARGS.includes("--sanctuary");
const NUMS = ARGS.filter((a) => !a.startsWith("--"));
if (SANCTUARY) BALANCE.mapSanctuary = true;
const POLICIES = SANCTUARY ? { ...MAP_POLICIES, ...SANCTUARY_MAP_POLICIES } : MAP_POLICIES;
...
function play(pick, seed) {
  ...
  const seen = { battle: 0, elite: 0, shop: 0, sanctuary: 0, boss: 0 };
...
console.log(
  `런 ${RUNS}회 · 구매·배치·개입 고정 · 길 고르기만 변경 · 시드 ${SEED0 + 1}~${SEED0 + RUNS}` +
    (SANCTUARY ? " · 성소 실험(정예 유물→성소)" : "") +
    "\n",
);
console.log(`정책          최소  p25  중앙값  p75  최대   평균   전투/정예/상점${SANCTUARY ? "/성소" : ""}`);
...
for (const name of Object.keys(POLICIES)) {
  const out = [];
  const tally = { battle: 0, elite: 0, shop: 0, sanctuary: 0, boss: 0 };
  perSeed[name] = [];
  ...
  means[name] = avg;
  const mix = `${(tally.battle / RUNS).toFixed(1)}/${(tally.elite / RUNS).toFixed(1)}/${(tally.shop / RUNS).toFixed(1)}` + (SANCTUARY ? `/${(tally.sanctuary / RUNS).toFixed(1)}` : "");
  console.log(
