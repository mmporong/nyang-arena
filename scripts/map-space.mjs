...
import { makeBossBot, walkMap, leaveShop, shopStep, MAP_POLICIES, EVENT_MAP_POLICIES } from "./bot-policy.mjs";
import { BALANCE } from "../src/game/balance.ts";

/**
 * `--events`: 제단(도박) 실험. 지도에 제단 노드가 나오고(`BALANCE.mapEvents`), 상태를 읽는
 * "제단 읽고 고름"이 눈감은 "제단 몰빵"·기존 정책들보다 나은지 본다. 플래그는 자리와 무관.
 */
const ARGS = process.argv.slice(2);
const EVENTS = ARGS.includes("--events");
const NUMS = ARGS.filter((a) => !a.startsWith("--"));
if (EVENTS) BALANCE.mapEvents = true;
const POLICIES = EVENTS ? { ...MAP_POLICIES, ...EVENT_MAP_POLICIES } : MAP_POLICIES;
...
function play(pick, seed) {
  ...
  const seen = { battle: 0, elite: 0, shop: 0, event: 0, boss: 0 };
...
console.log(`정책          최소  p25  중앙값  p75  최대   평균   전투/정예/상점${EVENTS ? "/제단" : ""}`);
...
for (const name of Object.keys(POLICIES)) {
  const out = [];
  const tally = { battle: 0, elite: 0, shop: 0, event: 0, boss: 0 };
  perSeed[name] = [];
  ...
  means[name] = avg;
  const mix = `${(tally.battle / RUNS).toFixed(1)}/${(tally.elite / RUNS).toFixed(1)}/${(tally.shop / RUNS).toFixed(1)}` + (EVENTS ? `/${(tally.event / RUNS).toFixed(1)}` : "");
  console.log(
