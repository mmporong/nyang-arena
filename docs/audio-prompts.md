...
**현재 상태 — BGM 4곡과 핵심 보스 신호 3종이 들어갔고, 나머지 SFX 17종은 아직이다.**

| | 상태 |
|---|---|
| BGM 4곡 | **완료.** 생성 → 측정 → 마스터링 → `public/bgm/` → `src/game/audio.ts` |
| 보스 신호 SFX 3종 | **구현.** 절차 합성 → 상태 상승 에지 관찰 → `src/game/audio.ts` |
| 나머지 SFX 17종 | 미착수. 프롬프트만 있다(§4) |
...
JavaScript 번들 크기는 계속 바뀌므로 `docs/generated/metrics-current.md`에서 잰 값을
기준으로 한다. 효과음은 파일을 추가하지 않고 코드로 합성해 에셋 전송량을 늘리지 않는다.
...
| SFX 20종 | 160 KB | 파일 0 KB · 코드만 증가 | 절차적 합성 (§4-0) |
| **합계** | **660 KB** | **452 KB** | |

**전부 지연 로드한다.** 첫 화면은 게임 번들과 스프라이트만으로 뜨고(현재 정확한
번들 값은 `docs/generated/metrics-current.md` 참조),
...
## 4. SFX 20종 — 핵심 3종 구현, 17종 대기

보스 신호 셋은 `src/game/audio.ts`에서 실시간 합성한다. 아래 프롬프트는 남은
17종을 만들 때도 같은 음향 언어를 유지하기 위한 명세다.
...
- [ ] 신호 셋을 눈 감고 구분할 수 있나 — `npm run qa:manual`의 연속 재생 1회만 판정
- [ ] 열 마리가 동시에 때릴 때 소음이 되지 않나 — 전투 SFX 17종 미착수

제출 직전 반복 검수는 `npm run qa:manual`로 줄였다. 이 명령이 오디오 런타임 계약과
공개 배포 응답을 자동으로 다시 확인하고, 실제 합성기에서 보스 BGM 위 세 신호를
순서대로 한 번 재생한다. 사람은 세 음색이 구분되는지만 답하면 된다.
