...
외부 에셋 팩 두 개를 조사하고 일부를 채택했다. **채택한 것보다 기각한 것이 훨씬
많으므로**, 나중에 같은 팩을 다시 열어 보는 사람이 같은 조사를 반복하지 않도록
전부 적어 둔다.

한 줄 결론: **Layer Lab 픽토그램 13장과 Canvas 손그림 4종만 쓴다.** 나머지는
스타일이 안 맞거나(풀컬러 광택), 이 게임 구조에 안 맞는다(Unity UI 전용 9-slice).
...
## 2. 채택 — 외부 픽토그램 13장 + Canvas 아이콘 4종

`public/icons/`에는 Layer Lab 원본과 정확히 대조되는 PNG 13장(**총 33.6 KB**)만
두고 `src/game/icons.ts`가 로드한다. 추가 4종은 같은 파일에서 Canvas 패스로 그린다.

| 쓰는 곳 | 아이콘 | 원본 |
|---|---|---|
| 화폐(HUD·카드 값·다시 뽑기) | 생선 | `fish_2` |
| 왼쪽 줄 직업 4종 | 전사·도적·궁수·마법사 | `warrior` `dagger_1` `archer` `magic_ball` |
| 오른쪽 줄 유물 8종 | 목걸이·발바닥·조준·별지팡이·사람·무리·왕관·보석 | `necklace` `paw` `target_1` `wand_star` `user` `friend` `crown_1` `diamond_1` |
| Canvas 손그림 4종 | 소환사·거울 부적·새끼 바구니·빈 방울 | `src/game/icons.ts`의 패스 |

**왜 이 갈래만 쓸 만한가.**

1. **흰 실루엣 + 투명 배경**이라 `source-in`으로 색을 갈아 끼울 수 있다. 이 게임은
   같은 그림을 직업색으로도(전사 붉은색 / 마법사 파란색), 상태색으로도(유물
   켜짐 금색 / 꺼짐 회색) 써야 한다. **색이 정보를 나르므로 틴트가 되는 것이 조건**이다.
2. 납작한 단색이라 이미 있는 UI 언어(손으로 그린 발톱·별·눈, `bevelPanel`)와 붙는다.
3. 128px 원본이라 20~40px로 줄여도 뭉개지지 않는다.
...
**폴백을 남겨 뒀다.** 생선은 로드에 실패하면 손으로 그린 패스로 떨어진다
(`icons.ts`의 `drawFish`). 자원 표시가 사라지면 무엇을 살 수 있는지 알 수 없게 된다.
...
- **저장소가 공개다.** 그래서 팩 전체를 넣지 않고 **실제로 쓰는 13장만** 넣었다.
  원본 팩(3,049장 / 978장)은 저장소 어디에도 없다
- 소환사·거울 부적·새끼 바구니·빈 방울은 `src/game/icons.ts`의 자체 Canvas
  패스라 별도 에셋 재배포나 출처 문제가 없다
- Addin 팩은 결과적으로 한 장도 쓰지 않았다
- 출처는 `README.md`의 라이선스 절에 적었다. Layer Lab 라이선스는 크레딧을
  요구하지 않지만, 공모전 제출물이라 **쓴 것은 쓴 대로 밝히는 편**이 맞다
